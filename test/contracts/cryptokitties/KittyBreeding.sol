...
contract KittyBreeding is KittyOwnership {
        ...
        returns(uint256)
    {
        ...
        uint256 cooldownEnd = matron.cooldownEndBlock - 1;
        uint256 childGenes = geneScience.mixGenes(matron.genes, sire.genes, cooldownEnd);

        // Make the new kitten!
        address owner = kittyIndexToOwner[_matronId];
        uint256 parentGenNew = parentGen + 1;
        uint256 siringW = matron.siringWithId;
        uint256 kittenId = _createKitty(_matronId, siringW, parentGenNew, childGenes, owner);
